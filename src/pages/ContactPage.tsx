...
const ContactPage = () => {
  ...
  // Get contact info from database with fallbacks
  const phone = getContent('info.phone', '+880 1410-190019');
  const email = getContent('info.email', 'agency.alphazero@gmail.com');
  const address = getContent('info.address', 'Hi-Tech Park, Rajshahi, Bangladesh');
  const hours = getContent('info.hours', 'Sat - Thu: 9:00 AM - 6:00 PM');
  const whatsappNumber = getContent('info.whatsapp', '+8801846484200');
  const whatsappDisplay = getContent('info.whatsapp_display', '+880 1846-484200');

  // Build WhatsApp link (strip spaces/dashes/+)
  ...
  const socialLinks = [
    ...
    { name: "WhatsApp", url: whatsappLink, icon: MessageCircle, comingSoon: false },
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-start gap-4 p-4 rounded-xl bg-secondary/50 border border-border hover:border-primary/30 transition-colors duration-300">
                  ...
                  <div>
                    ...
                    <a href={phoneLink} className="text-muted-foreground hover:text-primary transition-colors">
                      {phone}
                    </a>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-4 p-4 rounded-xl bg-secondary/50 border border-border hover:border-primary/30 transition-colors duration-300">
                  ...
                  <div>
                    ...
                    <a href={emailLink} className="text-muted-foreground hover:text-primary transition-colors">
                      {email}
                    </a>
                  </div>
                </div>
